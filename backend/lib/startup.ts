...
import { isNvidiaConfigured, nvidiaGenerateText } from './nvidia';
...
async function checkGeminiAI(): Promise<StartupCheckResult> {
  try {
    ...
    if (!isGeminiConfigured() || !apiKey) {
      log.warning('Gemini API key not configured (using NVIDIA/Mistral instead)');
      return {
        status: 'warning',
        component: 'Gemini AI',
...
async function checkNvidiaAI(): Promise<StartupCheckResult> {
  try {
    log.info('Checking NVIDIA AI configuration...');

    if (!isNvidiaConfigured()) {
      return {
        status: 'warning',
        component: 'NVIDIA AI',
        message: 'NVIDIA API key not configured (optional)',
      };
    }

    log.success('NVIDIA API key configured');

    const skipGenerateTest = /^(1|true|yes)$/i.test(String(process.env.NVIDIA_HEALTHCHECK_SKIP_GENERATE || ''));
    if (skipGenerateTest) {
      log.info('Skipping NVIDIA generate test (NVIDIA_HEALTHCHECK_SKIP_GENERATE enabled)');
      return {
        status: 'success',
        component: 'NVIDIA AI',
        message: 'API key is configured (generate test skipped)',
        details: { configuredModel: process.env.NVIDIA_MODEL || 'meta/llama-3.1-8b-instruct' },
      };
    }

    log.info('Testing NVIDIA model connection...');
    const text = await nvidiaGenerateText('Test connection. Respond with "OK".');
    if (text) {
      log.success('NVIDIA AI model is responsive');
      log.info(`  Model: ${process.env.NVIDIA_MODEL || 'meta/llama-3.1-8b-instruct'}`);
      return {
        status: 'success',
        component: 'NVIDIA AI',
        message: 'AI model is ready',
    ...
    log.error(`NVIDIA AI check failed: ${error.message}`);
    return {
      status: 'error',
      component: 'NVIDIA AI',
      message: `AI model unavailable: ${error.message}`,
...
export async function runStartupChecks(): Promise<StartupCheckResult[]> {
  ...
  // NVIDIA AI check
  results.push(await checkNvidiaAI());
