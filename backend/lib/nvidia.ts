...
export async function nvidiaGenerateTextWithOptions(
  ...
  const currentModelName = (process.env.NVIDIA_MODEL || 'meta/llama-3.1-8b-instruct').trim();
  const currentMaxTokens = clampNumber(parseEnvInt('NVIDIA_MAX_TOKENS', 256), 64, 1024);
  const currentTemperature = clampNumber(parseEnvFloat('NVIDIA_TEMPERATURE', 0.2), 0, 1);
